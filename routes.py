...
def update_expenditure(id):
    ...
    tags = request.json.pop("tags", [])
    expenditure.tags = [
        Tag.query.filter(Tag.name == tag["name"]).first() for tag in tags
    ]
...
def get_expenditure():
    ...
    if tag:
        selected_expenditures = selected_expenditures.join(
            tags, tags.columns.expenditure_id == Expenditure.id
    ...
    if year:
        selected_expenditures = selected_expenditures.filter(
            func.strftime("%Y", Expenditure.created_date) == str(year)
        )
...
def expenditure_summary():
    ...
    result_year = (
        db.session.query(
            func.strftime("%Y", Expenditure.created_date), func.sum(Expenditure.amount)
        )
...
def get_status():
    expenditure_sum = (
        db.session.query(Expenditure.username, func.sum(Expenditure.amount))
